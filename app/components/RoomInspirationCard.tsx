...
interface RoomInspirationCardProps {
  item: DrupalRoomInspiration
}

export default function RoomInspirationCard({ item }: RoomInspirationCardProps) {
  return (
    <Link
      href={item.path || '#'}
    ...
    >
      ...
      <div className="flex-1 p-6 flex flex-col justify-center">
        {(item as any).roomType && (
          <p className="text-sm text-accent-600 font-medium mb-1">{(item as any).roomType}</p>
        )}
