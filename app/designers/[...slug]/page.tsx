...
import { GET_DESIGNER_BY_PATH } from '@/lib/queries'
import { DrupalDesigner } from '@/lib/types'
...
interface DesignerByPathData {
  route: {
    entity: DrupalDesigner
  } | null
}

async function getDesigner(path: string): Promise<DrupalDesigner | null> {
  try {
    const requestHeaders = await headers()
    const apolloClient = getServerApolloClient(requestHeaders)
    const { data } = await apolloClient.query<DesignerByPathData>({
      query: GET_DESIGNER_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/designers/${slug.join('/')}`
  const item = await getDesigner(path)

  if (!item) {
    return { title: 'Designer Not Found | Furniture' }
  }
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  ...
                  {(item as any).education && (
                    <div>
                      <dt className="text-sm text-gray-500">Education</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).education}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
